    ...
    for event_id, event in events.items():
        if event.event_model_path.exists():
            try:
                ...
                print(e)
                print("\tCouldn't pull a fragment associated residue from event: {} {}. Skipping!".format(event.dtag,
                                                                                                       event.event_idx,
                                                                                                       ))
                continue

            event_model_path = pandda_fs_model.processed_datasets_dirs[
                                   event.dtag].modelled_structures_dir / "{}.pdb".format(
                event.event_idx)

            closest_lig.to_pdb(str(event_model_path))
        else:
            print("\tNo model for the event at {}! Passing!".format(event.event_model_path))
